...
function ratingFill(value){return (clampRating(value)/5*100).toFixed(1)+'%';}
...
function resolveDetailIndex(){
  ...
  if(current&&String(current[2]||'').trim()===albumText&&(!artistText||String(current[1]||'').trim()===artistText))return currentDetailIndex;
...
/* Community Rating matches the app's original definition: every user's rating
   counts in the average. Your Rating is stored separately for the signed-in user. */
...
function setRecordRatingMeta(albumId,entry){
...
function setOwnRatingOnly(albumId,rating){
  if(!Array.isArray(window.records))return;
  window.records.forEach(function(record){
...
function ensureRatingFooter(panel,ownRating){
  ...
  if(ownRating>0){
    if(!footer.querySelector('.groovy-remove-rating'))footer.innerHTML='<button class="groovy-remove-rating" type="button">Remove Rating</button>';
...
async function refreshLibraryRatings(){
  ...
  var result=await supabaseClient.from('album_ratings').select('album_id,user_id,rating').in('album_id',albumIds);
...
function scheduleLibraryRatingRefresh(delay){
  ...
  libraryRefreshTimer=setTimeout(refreshLibraryRatings,delay==null?100:delay);
}
...
async function refreshCurrentAlbumRatings(index){
  ...
  var user=await sessionUser();
  if(!user)return;
  var result=await supabaseClient.from('album_ratings').select('album_id,user_id,rating').eq('album_id',record[8]);
  if(result.error){console.warn('Could not refresh album rating:',result.error);return;}
  var map=calculateRatingMeta(result.data||[],user.id);
  var entry=map[String(record[8])]||{own:0,average:0,count:0};
  setRecordRatingMeta(record[8],entry);
...
async function saveCurrentRating(index,rating){
  ...
  setOwnRatingOnly(record[8],rating);
  updateDetailRatingDom(index);

  var result=await supabaseClient.from('album_ratings').upsert({
    user_id:user.id,
    album_id:record[8],
    rating:rating
  ...
  scheduleLibraryRatingRefresh(100);
}
...
async function removeCurrentRating(index,button){
  ...
  var result=await supabaseClient.from('album_ratings').delete().eq('user_id',user.id).eq('album_id',record[8]);
  if(result.error){
...
function syncMobileStreamingPlacement(){
  ...
  if(!mobile){if(existing)existing.remove();return;}
  if(!existing){existing=document.createElement('div');existing.className='groovy-mobile-streaming';infoCard.appendChild(existing);}
  existing.innerHTML=
    '<a class="groovy-mobile-streaming-link groovy-mobile-apple" href="'+escapeHtml(apple.href)+'" target="_blank" rel="noopener noreferrer" aria-label="'+escapeHtml(apple.getAttribute('aria-label')||'Listen on Apple Music')+'"><img src="/Apple_Music_Listen_on_Badge_Small.svg" alt="Listen on Apple Music"></a>'+
    '<a class="groovy-mobile-streaming-link groovy-mobile-spotify" href="'+escapeHtml(spotify.href)+'" target="_blank" rel="noopener noreferrer" aria-label="'+escapeHtml(spotify.getAttribute('aria-label')||'Listen on Spotify')+'"><img src="/Full_Logo_Green_RGB.svg" alt="Spotify"></a>';
}
...
function renderCollectedProfiles(profiles){
  ...
  function person(profile,extraClass){
    ...
    return '<button class="detail-social-person'+(extraClass?' '+extraClass:'')+'" type="button" data-detail-social-username="'+username+'" data-tooltip="'+username+'" aria-label="View '+username+'"><span class="detail-social-avatar" style="background-image:url(&quot;'+avatar+'&quot;)"></span><span class="detail-social-person-name">'+username+'</span></button>';
  }

  social.innerHTML='<div class="detail-social-heading"><strong>Collected by</strong><small>Collectors you follow</small></div><div class="detail-social-people">'+visible.map(function(profile){return person(profile,'');}).join('')+(hasMore?'<button class="detail-social-more" type="button" data-detail-social-more aria-expanded="false" aria-label="Show more collectors">…</button>':'')+'</div>'+(hasMore?'<div class="detail-social-menu" data-detail-social-menu hidden><div class="detail-social-menu-title">More collectors</div><div class="detail-social-menu-list">'+extra.map(function(profile){return person(profile,'detail-social-menu-person');}).join('')+'</div></div>':'');
}
...
async function refreshWishlistCollectedBy(index){
  ...
  var followResult=await supabaseClient.from('user_follows').select('followed_id').eq('follower_id',user.id);
  if(followResult.error)return;
  ...
  var query=supabaseClient.from('collections').select(masterId?'user_id,albums!inner(discogs_master_id)':'user_id').in('user_id',followedIds);
  query=masterId?query.eq('albums.discogs_master_id',masterId):query.eq('album_id',record[8]);
  var collectionResult=await query;
  if(collectionResult.error||token!==socialRequestToken)return;
  var ids=Array.from(new Set((collectionResult.data||[]).map(function(row){return row.user_id;}).filter(Boolean)));
  if(!ids.length){renderCollectedProfiles([]);return;}

  var profileResult=await supabaseClient.from('profiles').select('id,username,avatar_url').in('id',ids);
  if(profileResult.error||token!==socialRequestToken)return;
  var order=new Map(ids.map(function(id,pos){return [String(id),pos];}));
  var profiles=(profileResult.data||[]).slice().sort(function(a,b){return (order.get(String(a.id))||0)-(order.get(String(b.id))||0);});
...
async function addWishlistRecordToCollection(index,button){
  ...
  try{
    var existing=await supabaseClient.from('collections').select('id').eq('user_id',user.id).eq('album_id',record[8]).limit(1);
    if(existing.error)throw existing.error;
    if(!existing.data||!existing.data.length){
      var last=await supabaseClient.from('collections').select('sort_order').eq('user_id',user.id).order('sort_order',{ascending:false}).limit(1);
      if(last.error)throw last.error;
      var nextOrder=last.data&&last.data.length?(parseInt(last.data[0].sort_order,10)||0)+1:1;
      var inserted=await supabaseClient.from('collections').insert({user_id:user.id,album_id:record[8],cover_url:record[6]||null,discogs_style:record[4]||null,sort_order:nextOrder});
      if(inserted.error)throw inserted.error;
    }
    var removed=await supabaseClient.from('wishlists').delete().eq('id',record[9]).eq('user_id',user.id);
...
function cleanupLoggedOutPublicUi(){
  ...
  if(header&&header.style.display!=='none')header.style.display='none';
  if(header)header.dataset.own='false';
  var shelfStrip=document.getElementById('shelfStrip');
  if(shelfStrip&&!shelfStrip.hidden)shelfStrip.hidden=true;
  var search=document.getElementById('librarySearchInput');
  if(search&&search.value)search.value='';
}

function armLoggedOutCleanup(){
  loggedOutMode=true;
  cleanupLoggedOutPublicUi();
  [50,150,350,800,1600].forEach(function(delay){setTimeout(cleanupLoggedOutPublicUi,delay);});
}
...
function onDetailOpened(){
  ...
  if(window.libraryView==='wishlist'&&window.viewedUserId===null)setTimeout(function(){refreshWishlistCollectedBy(index);},80);
}
...
function install(){
  ...
  document.addEventListener('touchend',function(event){interceptRatingStar(event,true);},true);
  document.addEventListener('click',function(event){
    ...
    if(card){var cardIndex=parseInt(card.getAttribute('data-index'),10);if(!isNaN(cardIndex))currentDetailIndex=cardIndex;}
    var remove=event.target.closest&&event.target.closest('.groovy-remove-rating');
    if(remove){event.preventDefault();event.stopPropagation();var removeIndex=resolveDetailIndex();if(removeIndex>=0)removeCurrentRating(removeIndex,remove);return;}
    var add=event.target.closest&&event.target.closest('.groovy-detail-add-collection');
    if(add){event.preventDefault();event.stopPropagation();var addIndex=resolveDetailIndex();if(addIndex>=0)addWishlistRecordToCollection(addIndex,add);return;}
  },true);

  new MutationObserver(function(){scheduleLibraryRatingRefresh(100);}).observe(collection,{childList:true,subtree:true});
  new MutationObserver(function(){if(overlay.classList.contains('visible'))setTimeout(onDetailOpened,0);else socialRequestToken++;}).observe(overlay,{attributes:true,attributeFilter:['class']});

  if(header)new MutationObserver(function(){cleanupLoggedOutPublicUi();}).observe(header,{attributes:true,attributeFilter:['style','class','data-own']});
  if(shelfStrip)new MutationObserver(function(){cleanupLoggedOutPublicUi();}).observe(shelfStrip,{attributes:true,attributeFilter:['hidden','style','class']});

  window.addEventListener('resize',function(){placeRatingSection();syncMobileStreamingPlacement();if(overlay.classList.contains('visible'))setTimeout(onDetailOpened,0);},{passive:true});

  supabaseClient.auth.onAuthStateChange(function(event,session){
    if(session){loggedOutMode=false;}
    else armLoggedOutCleanup();
  });

  sessionUser().then(function(user){if(!user)armLoggedOutCleanup();});
  scheduleLibraryRatingRefresh(60);
