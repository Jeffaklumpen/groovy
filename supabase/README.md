...
Artist profiles use `/artist/:discogs-artist-id-:slug`. The browser renders Groovy statistics and the base discography first from `public.get_artist_overview(...)`; Discogs profile data and Wikipedia enrich the already-visible page asynchronously. Discogs profile metadata is cached in `public.artist_profile_cache` for fast repeat visits. Missing Discogs artist identity is resolved through the lightweight `resolveArtist` Edge Function action instead of waiting for the full artist-profile fetch, and album-detail prefetch starts identity/discography work before the user opens the artist route.

The base discography starts from the local MusicBrainz release-group catalog. Because that compact import intentionally omitted the information needed to distinguish a standardized core discography from every legitimate regional album, Groovy verifies Main Discography asynchronously. The artist identity is resolved server-side from the trusted Discogs artist ID to Wikidata's Discogs artist property. The same Wikidata lookup also resolves the English Wikipedia page when available, so first-time verification avoids a second identity round trip. Wikipedia remains the human-curated authority for which releases belong to Main Discography, but verification now prefers the compact Discography section on the artist's main article. When that section contains a usable core/studio list, Groovy needs only the main-article parse and does not follow the heavier dedicated discography page. The dedicated page remains a fallback for artists whose main article does not expose a usable list.

Groovy's local MusicBrainz→Discogs catalog enriches the Wikipedia-owned rows with stable identifiers. Obvious title matches are resolved first; a remaining title alias may use the only unused direct album from the same artist/year, and changed artist credits may fall back to a cross-catalog title+year match only when that match is unique. This handles cases such as Electric Light Orchestra → ELO → Jeff Lynne's ELO without allowing ambiguous global matches. Accepted albums are persisted in the shared `artist_discography_cache`, so after one successful verification every user can render that Main Discography directly from Postgres while any later refresh happens in the background. The verifier checks this shared cache before any Wikidata/Wikipedia request and reuses the persisted Wikidata identity when a refresh is eventually needed. Until verification exists, the fallback only accepts direct MusicBrainz→Discogs matches rather than fuzzy artist/title matches.

`get_artist_overview(...)` joins `apple_artwork_cache` directly, so artist pages and album search use persisted Apple artwork instead of calling Apple while rendering. Missing artist-discography artwork is warmed once in the background by the JWT-protected `discogs-search` Edge Function. Artwork warming is scoped to the already verified `artist_discography_cache`: it resolves the Apple artist once, loads the Apple album catalog once, matches only the albums that Groovy actually displays in Main Discography and persists the matching Apple album/artwork URLs. Later users therefore read both discography membership and Apple image URLs directly from the shared database caches. Recent incomplete warm attempts are remembered to avoid repeatedly hitting Apple for albums that have no match. Apple artwork cache writes are normalized to 1200×1200 at the database boundary, so legacy low-resolution Apple URLs cannot make artist cards blurry. Artist photos are not sourced from Discogs; the client only displays a Wikimedia Commons image when its metadata reports a free license, otherwise it uses the Groovy placeholder.
