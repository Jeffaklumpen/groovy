...
The base discography starts from the local MusicBrainz release-group catalog. Because that compact import intentionally omitted MusicBrainz primary type, Groovy verifies Main Discography asynchronously. The artist identity is resolved server-side from the trusted Discogs artist ID to Wikidata's Discogs artist property, then the English Wikipedia artist/discography page is used as the preferred human-curated core album list. Wikidata's structured studio-album data only enriches those Wikipedia-owned rows with stable identifiers; it does not decide membership. Accepted albums are persisted in the shared `artist_discography_cache`, so after one successful verification every user can render that Main Discography directly from Postgres while any later refresh happens in the background. The verifier checks this shared cache before any Wikidata/Wikipedia request and reuses the persisted Wikidata identity when a refresh is eventually needed. Until verification exists, the fallback only accepts direct MusicBrainz→Discogs matches rather than fuzzy artist/title matches.
