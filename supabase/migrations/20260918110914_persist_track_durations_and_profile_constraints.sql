-- Final foundation hardening before feature development.
alter table public.tracks add column if not exists duration text;
...
CREATE OR REPLACE FUNCTION public.save_album_to_library(p_destination text, p_discogs_master_id text, p_artist_name text, p_album_title text, p_release_year integer DEFAULT NULL::integer, p_genre text DEFAULT NULL::text, p_cover_url text DEFAULT NULL::text, p_cover_source text DEFAULT NULL::text, p_apple_collection_url text DEFAULT NULL::text, p_tracks jsonb DEFAULT '[]'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
begin
  ...
  if v_master_id = '' or v_master_id !~ '^[0-9]{1,20}$' then
    raise exception 'Invalid Discogs master ID';
  end if;

  if v_artist_name = '' or v_album_title = '' then
    raise exception 'Artist and album title are required';
  end if;

  if v_release_year is not null and (v_release_year < 1800 or v_release_year > 2100) then
    v_release_year := null;
  end if;

  if v_cover_url is not null and v_cover_url !~* '^https://' then
    raise exception 'Invalid cover URL';
  end if;

  if v_apple_url is not null and v_apple_url !~* '^https://(music|itunes)\.apple\.com/' then
    raise exception 'Invalid Apple Music URL';
  end if;

  if p_tracks is null then
    p_tracks := '[]'::jsonb;
  end if;
  ...
  -- Serialize creation of the same artist and album without blocking unrelated saves.
  ...
  for v_track in
    select value from jsonb_array_elements(p_tracks)
  loop
    v_track_side := upper(left(btrim(coalesce(v_track->>'disc_side','')),1));
    if v_track_side not in ('A','B','C','D','E','F','G','H') then
      continue;
    end if;
    ...
    if v_track_number is not null and (v_track_number < 1 or v_track_number > 999) then
      v_track_number := null;
    end if;
    ...
    v_track_duration := nullif(left(btrim(coalesce(v_track->>'duration','')),20),'');

    update public.tracks t
    set duration = coalesce(nullif(btrim(t.duration),''),v_track_duration)
    ...
    if not found then
      insert into public.tracks(album_id,disc_side,track_number,title,duration)
      values(v_album_id,v_track_side,v_track_number,v_track_title,v_track_duration);
    end if;
  end loop;

  -- Serialize ordering for this user's destination so concurrent inserts cannot
  -- both claim the same next sort_order.
  ...
  if v_destination = 'wishlist' then
    ...
    where w.user_id = v_user_id
      and w.album_id = v_album_id
    limit 1;

    if v_entry_id is null then
      select coalesce(max(w.sort_order),0)+1
      into v_next_order
      from public.wishlists w
      where w.user_id = v_user_id;
      ...
      values(
        v_user_id,v_album_id,v_cover_url,v_genre,v_next_order
      )
...
create or replace function public.save_album_to_library_verified(
  p_user_id uuid,
  p_destination text,
...
begin
  ...
  -- Library rows have their own optional cover override. Seed that override from
  -- the final persisted album cover, not from the raw incoming candidate. This
  -- prevents a transient Apple verification failure from downgrading an
  -- existing verified Apple cover to a Discogs image in collection/wishlist.
  select a.cover_url into v_cover_url
