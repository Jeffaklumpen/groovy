create table if not exists public.artist_profile_cache (
  discogs_artist_id bigint primary key check (discogs_artist_id > 0),
  artist_name text not null,
  current_members jsonb not null default '[]'::jsonb,
  past_members jsonb not null default '[]'::jsonb,
...
create index if not exists artist_profile_cache_name_idx
  on public.artist_profile_cache(lower(btrim(artist_name)));

alter table public.artist_profile_cache enable row level security;

revoke all on table public.artist_profile_cache from public, anon, authenticated;
grant select on table public.artist_profile_cache to authenticated;
grant all on table public.artist_profile_cache to service_role;

drop policy if exists "Authenticated users can read artist profile cache"
  on public.artist_profile_cache;

create policy "Authenticated users can read artist profile cache"
  on public.artist_profile_cache
  for select
  to authenticated
  using (true);
