create table if not exists public.artist_discography_cache (
  discogs_artist_id bigint not null check (discogs_artist_id > 0),
  mbid uuid not null,
  discogs_master_id bigint not null check (discogs_master_id > 0),
  album_title text not null,
  first_release_year smallint,
  source text not null default 'wikidata' check (source in ('wikidata','manual')),
  verified_at timestamptz not null default now(),
  primary key (discogs_artist_id,mbid)
);

create index if not exists artist_discography_cache_master_idx
  on public.artist_discography_cache(discogs_master_id);

alter table public.artist_discography_cache enable row level security;

revoke all on table public.artist_discography_cache from public,anon,authenticated;
grant select on table public.artist_discography_cache to authenticated;
grant all on table public.artist_discography_cache to service_role;

drop policy if exists "Authenticated users can read artist discography cache"
  on public.artist_discography_cache;

create policy "Authenticated users can read artist discography cache"
  on public.artist_discography_cache
  for select
  to authenticated
  using (true);
...
alter table public.artist_profile_cache
  ...
  add column if not exists discography_checked_at timestamptz,
  add column if not exists discography_source text,
  add column if not exists discography_count integer not null default 0;
