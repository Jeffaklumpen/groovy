...
update public.artist_discography_cache
set source_key = coalesce(
  source_key,
  case
    when mbid is not null then 'mbid:' || mbid::text
    else 'legacy:' || md5(lower(btrim(album_title)) || '|' || coalesce(first_release_year::text,''))
  end
)
...
with ranked as (
  select
    ...
    source_key,
    row_number() over (
      partition by discogs_artist_id
      order by first_release_year nulls last, album_title, source_key
    )::smallint as position
  from public.artist_discography_cache
)
update public.artist_discography_cache adc
set position = ranked.position
from ranked
where adc.discogs_artist_id = ranked.discogs_artist_id
  and adc.source_key = ranked.source_key
  and adc.position is null;

alter table public.artist_discography_cache
  drop constraint if exists artist_discography_cache_pkey,
  drop constraint if exists artist_discography_cache_discogs_master_id_check;

alter table public.artist_discography_cache
  alter column mbid drop not null,
  alter column discogs_master_id drop not null,
  alter column source_key set default ('row:' || gen_random_uuid()::text),
  alter column source_key set not null,
  alter column position set default 0,
  alter column position set not null;

alter table public.artist_discography_cache
  add constraint artist_discography_cache_discogs_master_id_check
    check (discogs_master_id is null or discogs_master_id > 0),
  add constraint artist_discography_cache_position_check
    check (position >= 0),
  add constraint artist_discography_cache_pkey
    primary key (discogs_artist_id, source_key);
...
declare
  ...
  v_discography_source text := 'unchecked';
begin
  ...
  select coalesce(apc.discography_source,'unchecked')
    into v_discography_source
  from public.artist_profile_cache apc
  where apc.discogs_artist_id=v_discogs_artist_id
  limit 1;

  v_discography_source:=coalesce(v_discography_source,'unchecked');
  if v_discography_verified and v_discography_source='unchecked' then
    v_discography_source:='verified';
  end if;
  ...
  with verified_rows as (
    select
      adc.mbid,
      adc.discogs_master_id,
      adc.album_title,
      adc.first_release_year,
      null::text as secondary_types,
      adc.position::bigint as sort_position,
      1 as rn
  ...
  fallback_candidates as (
    select
      mb.mbid,
      mb.discogs_master_id,
      mb.album_title,
      mb.first_release_year,
      nullif(btrim(mb.secondary_types),'') as secondary_types,
      row_number() over (
        partition by coalesce(
          mb.discogs_master_id::text,
          lower(btrim(mb.album_title))||'|'||coalesce(mb.first_release_year::text,'')
        )
        order by mb.mbid
      ) as rn
  ...
  fallback_rows as (
    select
      fc.mbid,
      fc.discogs_master_id,
      fc.album_title,
      fc.first_release_year,
      fc.secondary_types,
      row_number() over (
        order by fc.first_release_year nulls last, fc.album_title, fc.mbid
      )::bigint as sort_position,
      fc.rn
  ...
  rows as (
    ...
    from source_rows s
    left join public.albums a
      on s.discogs_master_id is not null
     and a.discogs_master_id=s.discogs_master_id::text
    left join public.apple_artwork_cache aac
      on s.discogs_master_id is not null
     and aac.discogs_master_id=s.discogs_master_id
    left join rating_summary rs on rs.album_id=a.id
    where s.rn=1
    order by s.sort_position,s.first_release_year nulls last,s.album_title
    limit 60
  )
  select coalesce(jsonb_agg(jsonb_build_object(
    ...
    'apple_collection_url',r.apple_collection_url,
    'apple_artwork_cached',r.apple_artwork_cached,
    'average_rating',r.average_rating,
    'rating_count',r.rating_count,
    'is_collected',r.is_collected,
    'is_wishlisted',r.is_wishlisted
  ) order by r.sort_position,r.year nulls last,r.title),'[]'::jsonb),
  ...
  return jsonb_build_object(
    ...
    'discography_source',v_discography_source,
...
where discography_checked_at is not null
   ...
   or coalesce(discography_count,0)<>0;
