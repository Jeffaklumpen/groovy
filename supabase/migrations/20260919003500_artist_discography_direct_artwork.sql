alter table public.artist_discography_cache
  ...
  add column if not exists apple_collection_url text,
...
begin
  ...
  with verified_rows as (
    select
      ...
      adc.artwork_url as cached_cover_url,
      adc.apple_collection_url as cached_apple_url,
      (adc.apple_collection_id is not null and adc.artwork_url is not null) as direct_apple_artwork_cached,
  ...
  fallback_candidates as (
    select
      ...
      null::text as cached_cover_url,
      null::text as cached_apple_url,
      false as direct_apple_artwork_cached,
      row_number() over (
  ...
  fallback_rows as (
    select
      ...
      fc.cached_cover_url,
      fc.cached_apple_url,
      fc.direct_apple_artwork_cached,
      row_number() over (
  ...
  rows as (
    select
      ...
      s.sort_position,
      a.id as album_id,
      coalesce(aac.artwork_url,s.cached_cover_url,a.cover_url) as cover_url,
      coalesce(aac.apple_collection_url,s.cached_apple_url,a.apple_collection_url) as apple_collection_url,
      (aac.discogs_master_id is not null or s.direct_apple_artwork_cached) as apple_artwork_cached,
